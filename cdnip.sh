...
if [ -z "$token" ]; then
    ...
    response=$(curl -s --max-time 8 "$url")
    if [ $? -eq 0 ]; then
        echo "Pushplus已推送"
    else
        echo "Pushplus无响应或超时，推送失败"
    fi
fi
}
...
for ((i=1; i<=$max_retries; i++)); do
    ...
    if [[ $resSuccess == "true" ]]; then
        ...
        pushmessage="尝试三次登陆错误,检查Cloudflare邮箱、区域ID、API Key，这三者的信息填写是否正确！"
        tgaction
        exit
    else
...
params="name=${subdomain}.${domain}&type=A"
response=$(curl -s -X GET "$url?$params" -H "X-Auth-Email: $x_email" -H "X-Auth-Key: $api_key")
if [[ $(echo "$response" | jq -r '.success') == "true" ]]; then
    ...
    if [[ $(echo "$records" | jq 'length') -gt 0 ]]; then
        for record in $(echo "$records" | jq -c '.[]'); do
            ...
            if [[ $(echo "$delete_response" | jq -r '.success') == "true" ]]; then
                ...
                echo "删除 DNS 记录失败：$(echo "$delete_response" | jq -r '.errors[0].message')"
            fi
        done
    else
        echo "没有找到指定的 DNS 记录"
    ...
    echo "获取 DNS 记录失败：$(echo "$response" | jq -r '.errors[0].message')"
fi
csv_file='result.csv'
if [[ -f $csv_file ]]; then
    ...
    for ip in $ips; do
        ...
        if [[ $(echo "$response" | jq -r '.success') == "true" ]]; then
            ...
            echo "导入IP地址 $ip 失败：$(echo "$response" | jq -r '.errors[0].message')"
        fi
    done > informlog
else
