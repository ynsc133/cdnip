...
if [ "$IP_ADDR" = "ipv6" ] ; then
    ./cfst -tp $point $CFST_URL_R -t $CFST_T -n $CFST_N -dn $CFST_DN -tl $CFST_TL -tll $CFST_TLL -sl $CFST_SL -f ipv6.txt $CFST_SPD
    else
    ./cfst -tp $point $CFST_URL_R -t $CFST_T -n $CFST_N -dn $CFST_DN -tl $CFST_TL -tll $CFST_TLL -sl $CFST_SL $CFST_SPD
fi
...
echo "比如每天早上三点自动执行：0 3 * * * cd /root/cfipopw/ && bash cdnip.sh"
echo
rm -rf informlog
exit 0;
